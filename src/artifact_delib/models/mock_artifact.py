...
_R1 = {
    ...
    "self_consistency": (
        "综合各维度视觉特征，该器物呈现明代早期青花梅瓶的典型特征。"
        "器形为梅瓶，小口短颈丰肩；纹饰为缠枝莲，青花发色浓艳；"
        "釉面肥润，工艺精细，整体风格指向永乐时期。"
    ),
    "multi_agent_debate": (
        "基于图像视觉特征，这是一件明代早期青花梅瓶。"
        "器形、纹饰和釉面特征共同指向永乐时期的制品。"
    ),
    "generic_expert": (
        "综合视觉分析：该器物为一件青花瓷梅瓶。"
        "器形规整，小口短颈丰肩，腹部线流畅。"
        "纹饰以缠枝莲为主题，青花发色浓艳。"
        "材质为瓷质，胎体较薄，釉面肥润光泽。"
        "从整体风格判断，更符合明代永乐时期的特征。"
    ),
    "generic_recheck": (
        "经重新审视，器形、纹饰和细节特征相互印证。"
        "坚持原判断，该器物为明代永乐时期青花梅瓶。"
    ),
    "free_debate": (
        "根据现有视觉证据，该器物为明代早期青花梅瓶。"
        "多个维度的特征指向永乐时期，虽有一些宣德特征但仍以永乐为主。"
    ),
    "hypothesis_agent": (
        "基于现有专家分析，我支持当前的候选判断。"
        "器形、纹饰和局部细节都指向这一方向。"
